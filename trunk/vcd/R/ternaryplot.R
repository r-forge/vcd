          ...
          ...)
{
  ...
  if (dimnames.position == "edge") {
    ...
    grid.text(x = 0.25 - 2 * eps - shift, y = 0.5 * top + shift,
              label = dimnames[2], rot = 60, gp = gpar(col = dimnames.color))
    grid.text(x = 0.75 + 3 * eps + shift, y = 0.5 * top + shift,
              label = dimnames[1], rot = -60, gp = gpar(col = dimnames.color))
    grid.text(x = 0.5, y = -0.02 - shift,
              label = dimnames[3], gp = gpar(col = dimnames.color))
  }
  ...
  if (is.character(grid))
    for (i in 1:4 * 0.2) {
      ...
      grid.lines(c(1 - i , (1 - i) / 2), c(0, 1 - i) * top,
                 gp = gpar(lty = grid, col = grid.color))
      ## b - axis
  ...
  xp <- x[,2] + x[,3] / 2
  yp <- x[,3] * top
  size = unit(if(prop.size) prop.size * (s / max(s)) else cex, "lines")
