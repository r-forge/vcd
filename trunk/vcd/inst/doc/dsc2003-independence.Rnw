...
%\VignetteIndexEntry{Visualizing Independence Using Extended Association and Mosaic Plots}
%\VignetteDepends{vcd,MASS,grid}
%\VignetteKeywords{mosaic plot, association plot, independence, HCL colors}
%\VignettePackage{vcd}
...
\author{\hfill Achim Zeileis$^\dag$ \hfill David Meyer$^\dag$ \hfill Kurt Hornik$^\ddag$ \hfill \hfill \\[0.2cm]
...
grid.mosaicpairs(UCBAdmissions, type = "conditional", permute = FALSE)
@
\caption{Extended conditioned association plots for the UCB Admissions data.}
