...
color implementation from package \pkg{colorspace} \citep{vcd:Ihaka:2004}. This gives the user
...
of a geyser eruption and the waiting time for this eruption.
Figure~\ref{fig:bkde} shows (two versions of) a heatmap of a bivariate kernel density estimate of 
duration and waiting time revealing a multi-modal bivariate distribution: short waiting
...
or short eruption (around 4 minutes). Both versions of the heatmap use a sequential
...
and transforms them to RGB by calling the \code{hcl()} function 
