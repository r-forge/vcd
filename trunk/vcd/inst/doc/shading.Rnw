...
\noindent As the example shows, we therefore create a fourfold table 
with appropriate colors and supply them to the \code{fill} component 
of the \code{gpar} object passed to the \code{gp} argument of \code{mosaic()}. 
In addition, we use the \code{col} component to turn off the borders 
...
color in the `Hair and Eye Color' data set (see Figure \ref{fig:haireye})
...
following example using the `Bundesliga' data 
shows the relationship of home goals and away
goals of the German football league in 1995: although there are two
