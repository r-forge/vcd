...
\item Providing a grapcon function to the \code{gp} argument that 
  ...
  alternative 1.
\item Providing a grapcon \emph{generating} function taking parameters and returning
  a function as described in alternaive 2. 
\end{enumerate}
...
\noindent Note the \code{class} attribute set on \codefun{shading2\_fun}.
To create a mosaic display with binary shading, 
...
\subsubsection*{Alternative 3: grapcon-generating function}
...
using the new \codefun{shading3a\_fun} function, we can now simply change the colors:
...
Now for the shading functions, more customization is needed. Note that
\codefun{shading3a\_fun} needs to be evaluated by the user, even if the
defaults shall be used. It is a better idea to let \codefun{strucplot} call
the generating function, which, in particular, allows the passing of arguments that
are computed by \codefun{strucplot}. Since shading
functions can be used for visualizing significance (see Section 
\ref{sec:overview}), it makes sense for generating functions to have access to the model,
