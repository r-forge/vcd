...
mosaic displays and association plots. The main idea is to visualize
...
low-level panel functions for association plots (\code{struc\_assoc}) and 
mosaic plots (\code{struc\_mosaic}) are available. In addition, there are
high-level wrapper functions such as \code{mosaic()} and \code{assoc()} 
to \code{strucplot()} that conveniently set the 
parameters---it is these high-level functions that the user normally would like to use, 
although \code{strucplot()} is fully functional on its own. \code{mosaic()} and 
\code{assoc} are generic functions: for both, there exist a formula method and a default 
method. Other functions, such as \code{doubledecker()}, \code{pairs()}, and 
\code{cotabplot}, are on even higher level and make use of 
code{mosaic()} and \code{assoc()} to produce plots.
...
<<structable,fig=FALSE,echo=TRUE,height=6,width=7,results=verbatim>>=
structable(Titanic)
@

Let us
first visualize the contingency table by the means of a mosaic plot
...
\begin{figure}[h]
\begin{center}
<<Observed,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
\begin{center}
<<Expected,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
mosaic(Titanic, type = "expected", main = TRUE, sub = "Expected values")
@
...
\noindent The Pearson residuals (standardized deviations of observed from expected values) 
...
\begin{figure}[h]
\begin{center}
<<Residuals,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
assoc(Titanic, compress = FALSE)
@
...
\begin{figure}[h]
\begin{center}
<<Variables,fig=TRUE,echo=FALSE,height=6,width=16,results=hide>>=
pushViewport(viewport(layout = grid.layout(ncol = 2)))

pushViewport(viewport(layout.pos.col = 1))
mosaic(~ Class + Age, data = Titanic[,"Male",,], 
       margin = c(bottom = 0), sub = "Male", newpage = FALSE,
       labeling_args = list(gp_labels = gpar(fontsize = 24), 
...
\begin{center}
<<Conditioning,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
mosaic(~ Class + Age | Sex, data = Titanic, split = TRUE)
@
\caption{Mosaic plot for the Titanic data (expected values).}
...
\noindent Note, however, that the plots in the ``pseudo-strata'' are
...
\begin{figure}[h]
\begin{center}
<<split1,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
mosaic(Titanic, direction = "h")
@
...
\begin{figure}[h]
\begin{center}
<<split2,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
mosaic(Titanic, split_vertical = c(TRUE, TRUE, TRUE, FALSE),
...
\begin{figure}[h]
\begin{center}
<<doubledecker,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
doubledecker(Titanic)
@
...
\begin{figure}[h]
\begin{center}
<<pairs,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
pairs(Titanic, upper_panel = pairs_assoc)
@
...
\begin{figure}[h]
\begin{center}
<<cotabplot,fig=TRUE,echo=FALSE,height=6,width=12,results=hide>>=
cotabplot(~ Class + Survived | Sex + Age, data = Titanic)
@
...
to all parameters passed to the generating function thanks to 
lexical scope. The idea is to use module-specific `code
...
\begin{figure}[h]
\begin{center}
<<afterplot,fig=TRUE,echo=FALSE,height=6,width=12,results=hide>>=
mosaic(~ Sex + Class, data = Titanic, pop = FALSE)
...
\begin{figure}[h]
\begin{center}
<<changeplot,fig=TRUE,echo=FALSE,height=6,width=12,results=hide>>=
assoc(Titanic)
getNames() ## output not shown
grid.edit("rect..Class.1st..Age.Adult..Sex.Female..Survived.Yes", 
          gp = gpar(fill = "red"))
