...
\SweaveOpts{engine=R,eps=TRUE,prefix.string=pics/strucplot}
...
\begin{figure}[h]
\begin{center}
<<afterplot,fig=TRUE,echo=FALSE,height=6,width=6,results=hide>>=
mosaic(~ Sex + Class, data = Titanic, pop = FALSE)
...
\begin{figure}[h]
\begin{center}
<<changeplot,fig=TRUE,echo=FALSE,height=6,width=6,results=hide>>=
assoc(Titanic)
x = Titanic
x[] = NA
