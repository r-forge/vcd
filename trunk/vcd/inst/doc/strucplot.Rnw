...
appearance of the plot. These `graphical appearance control'
(`grapcon') functions are created by generating functions, allowing
flexible parameterization and extensibility. 
The first part of their names (\code{\emph{group\_foo()}}) 
reflects the group they belong to (strucplot core, labeling,
legend, shading, or spacing). The workhorse functions (created by 
\code{\emph{struc\_foo()}}, 
...
(created by \code{\emph{spacing\_foo()}} and \code{\emph{shading\_foo()}}) compute
graphical parameters used by the others. The grapcon functions returned by 
\code{\emph{struc\_foo()}} implement the core functionality, creating the tiles and their
content.  On the second level of the framework, a suitable combination
of grapcon functions (or, alternatively, corresponding generating functions)
