...
\SweaveOpts{engine=R,eps=FALSE,prefix.string=pics/labeling}
...
overlap. This issue can be addressed in several ways. The `brute force'
...
and therefore clipping is enabled only for the \code{Survived} dimension.
A more sensible solution to the overlap problem is to abbreviate the `Survival'
...
abbreviated to, but logical values are equivalent to 1. Abbreviation
...
\noindent The \code{"tl\_foo"} (``top left'') arguments are \code{TRUE} by default. 
...
> mosaic(Titanic, labeling_args = list(tl_labels = TRUE, boxes = TRUE, 
         clip = TRUE, labbl_varnames = TRUE), margins = c(left = 4, right = 1, 3))
\end{Sinput}
...
> mosaic(~ PremaritalSex + ExtramaritalSex, data = PreSex, 
         ...
         abbreviate_varnames = TRUE, clip = F))
\end{Sinput}
