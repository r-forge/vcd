...
\title{Labeling in the Strucplots Framework}
...
offers much more features and flexibility. Like the core, 
shading, spacing, and legend modules, labeling is now completely
modular (see the introducing vignette for an overview on
...
functions is discussed in detail in the vignette on colors and shadings). 
...
`\texttt{dimnames}' attribute of the visualized table. The drawing of the
...
<<default>>
@
\caption{Mosaic plot for the \data{Titanic} data.}
\label{fig:defaults}
\end{center}
\end{figure}
...
also be achieved by using \codefun{labeling\_left} (see Figure \ref{fig:left}):
...
all dimensions. The result is pretty close to what the \codefun{labels\_cboxed}
...
variable names. The resulting layout is basically what \codefun{labeling\_lboxed} produces,
and a similar design is used by the \codefun{doubledecker} function.
