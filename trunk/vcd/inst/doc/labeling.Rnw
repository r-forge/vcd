...
base {\sf R} is the labeling in the strucplot framework which now
...
to \codefun{strucplot} which then draws the labels (generating
functions are discussed in a separate vignette). The labeling is 
...
% As an example, consider \codefun{labeling\_list}:
...
% \codefun{labeling\_list} is a generating function that takes all parameters
...
and a similar design is used by the \codefun{doubledecker}.
