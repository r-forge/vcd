...
\begin{table}
  \begin{tabular}{|l|l|l|}
    ...
    strucplot & \codefun{struc\_mosaic} & core function for mosaic plots\\
    core          & \codefun{struc\_assoc} & core function for association plots\\
     ...
     labeling & \codefun{labeling\_border} & border labels\\
              ...
              && and on top and left border\\\hline\hline
     shading  & \codefun{shading\_hsv} & shading based on HSV-colors\\
              ...
              & \codefun{shading\_binary} & visualizes the sign of the
              residuals\\\hline\hline
     spacing  & \codefun{spacing\_equal} & equal spacing for all dimensions\\
              & \codefun{spacing\_dimequal} & equal spacing for each dimension\\
              & \codefun{spacing\_increase} & increasing spacing\\
              & \codefun{spacing\_highlighting} & increasing spacing,
              last dimension set to zero\\
              & \codefun{spacing\_conditional} & increasing spacing
              for conditioning variables,\\&& equal spacing for
              conditioned variables.\\\hline\hline
     legend   & \codefun{legend\_resbased} & suitable for an
     arbitrary number of bins\\&& (also for continuous shadings)\\
              & \codefun{legend\_fixed} & creates a fixed number of
              bins (similar to \codefun{mosaicplot})\\\hline
