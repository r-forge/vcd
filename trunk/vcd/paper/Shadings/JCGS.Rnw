...
if(recompute) {
  ...
  save(art_max, art_chisq,
       ...
       pun_dmax, pun_maxchisq, pun_sumchisq, pun_cotab,
...
$\hat n_{ij} = n_{i+}n_{+j}/n_{++}$.
%FIXME% point out homogeneity problem?
...
from a sample of size $\Sexpr{nrep}$ from the permutation distribution under independence
...
\citep{vcd:R:2005} employs colors from HSV space. 
...
The HSV space looks like a cone\footnote{The wikipedia gives a very useful
%FIXME% proper cite for Wikipedia
...
as assessed by the symmetric difference of the planes, to that of $h = 0$.}
...
of visualization and corresponds more closely with the properties of the Friendly shading.
...
\begin{figure}[p]
\begin{center}
<<shadings,fig=TRUE,echo=FALSE,height=12,width=16,results=hide,include=FALSE>>=
...
To see such a sum-of-squares shading in practice, we employ a data on the number of piston
ring failures in three legs (north/center/south) in four different steam-driven compressors
at an FIXME. The contingency table is given in \cite{vcd:Haberman:1973}, re-analyzed
...
evidence for a departure from independence in the data (at 5\% level). However, as argued
...
shows again that the latter are less flashy and more balanced.
...
\begin{figure}[b!]
\begin{center}
<<conditional_mosaic,fig=TRUE,echo=FALSE,height=5,width=9>>=
...
suggested above find evidence for departure from significance: the sum-of-squares
...
The $p$ values are again computed by drawing $\Sexpr{nrep}$ samples from the corresponding
...
\begin{figure}[t!]
\begin{center}
<<conditional_assoc,fig=TRUE,echo=FALSE,height=11,width=11>>=
...
As a 4-way example, we use the punishment data \cite{vcd:andersen:1991} 
...
indpendence model. It shows an association between memories and attitude
...
\pkg{vcd}~\Sexpr{packageDescription("vcd")$Version} \citep{vcd:Meyer+Zeileis+Hornik:2006},
