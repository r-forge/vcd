...
and thus do not have the problem of varying saturations that the HSV colors have.
...
dependence. Currently, both are not true as can be seen in the left panel
...
to both the maximum and $\chi^2$ tests), no residual exceeds an absolute value of $2$
...
$\sum_k X_k^2 = \Sexpr{stat(alz_sumchisq)}$ ($p = \Sexpr{pval(alz_sumchisq)}$),
the maximum sum-of-squares statistic from Equation~\ref{eq:maxchisq} is
$\max_k X_k^2 = \Sexpr{stat(alz_maxchisq)}$ ($p = \Sexpr{pval(alz_maxchisq)}$),
and the double maximum statistic from Equation~\ref{eq:dmax} is
$\max_k M_k = \Sexpr{stat(alz_dmax)}$ ($p = \Sexpr{pval(alz_dmax)}$).
