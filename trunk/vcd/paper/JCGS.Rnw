...
statistic $M$ (\ref{eq:MaxAbs}) which always employs the critical values $c_\alpha$
...
(and using the diverging HCL palette). By using these cut offs, the presence of color
...
but it should only be applied when using the maximum statistic is appropriate. For 
the sum-of-squares statistic $X^2$ (and all other functionals $\lambda(\cdot)$), this
shading cannot be used.

use pre-determined (or manually adjusted) cut offs, but visualize the significance
by the amount of color in the plot, i.e., use only little color if the corresponding
test result is non-significant.

For HCL, the amount of color can easily be controlled by varying the maximum
chroma value. So far $100$, now $20$.

For HSV, a similar effect can be achieved by varying the value dimension which
was not used up to now. Setting value to $v = 0.5$ a darker palette of colors can be
obtained.
