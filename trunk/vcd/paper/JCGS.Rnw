...
result of an associated significance test is coded by the presence of
...
for multi-way tables, including in particular the conditional independence
problem.\\[0.3cm]
...
independence and mosaic plots \citep{vcd:Hartigan+Kleiner:1981} for 
bringing it out graphically. Both methods can also be combined such that
...
shading is extended such that presence of color in the display is equivalent to significance
...
them to conditional independence. Section~\ref{sec:conclusions} summarizes the paper
...
suitable for identifying the cells responsible for the dependence (if any) is
...
$2$ and $4$. The problem is that it is not clear to which
...
\section{Extension to Conditional Independence} \label{sec:3way}
...
independence problem are derived in the following.
