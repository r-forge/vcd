...
\begin{center}
\textbf{\LARGE Residual-based Shadings for Visualizing (Conditional) Independence}
...
## pmos.Friendly <- function(x, ...) grid.mosaicplot(x, panel = TRUE, legend = FALSE,
##   labels = FALSE, space = 0.02, gp = gp.Friendly, ...)
## pmos.max <- function(x, ...) grid.mosaicplot(x, panel = TRUE, legend = FALSE,
##   labels = FALSE, space = 0.02, gp = gp.max, ...)
...
Arthritis data \citep{vcd:Koch+Edwards:1988}
...
\section{Visualizations} \label{sec:visualization}
...
Control of Trellis Display'', Journal of Computational and
...
mosaic(emp1, split_vertical = TRUE)
@
\caption{Classic association and mosaic plot for the ``Replaced'' subset of the Employment data.}
\label{fig:classic}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
<<UCB-mosaic1,fig=TRUE,echo=FALSE,height=6,width=8>>=
## fm <- loglm(~(EmploymentStatus+EmploymentLength)*LayoffCause, data = emp, fitted = TRUE)
## my.gp <- gp.HCLshading(emp, residuals(fm, type = "pearson"))
## my.gp$p.value <- summary(fm)$tests[2,3]
## my.gp$lty <- c(1,1)
## grid.mosaicplot(emp, gp = my.gp, margin = ~(EmploymentStatus+EmploymentLength)*LayoffCause,
##   space = c(0.1, 0.01, 0), dir = "v")
mosaic(emp, expected = ~(EmploymentStatus+EmploymentLength)*LayoffCause)
@
\caption{Employment data with mosaic display.}
...
\begin{figure}[htbp]
\begin{center}
<<emp1-Friendly,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>
mosaic(emp1, gp = shading_Friendly)
@
<<emp1-max,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>
mosaic(emp1, gp = shading_max)
@
\caption{Mosaic plots for the ``Replaced'' subset of the Employment data.}
\label{fig:emp1}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
\begin{center}
<<UCB-mosaic2,fig=TRUE,echo=FALSE,height=4,width=8>>
tabplot(emp, panel = pmos.max)
@
\caption{Employment data with conditioned mosaic display.}
\label{fig:emp-mosaic2}
\end{center}
\end{figure}

\begin{figure}[htbp]
\begin{center}
<<UCB-assoc,fig=TRUE,echo=FALSE,height=4,width=8>>
tabplot(emp)
@
\caption{Employment data with conditioned association plot.}
\label{fig:emp-assoc}
\end{center}
\end{figure}
