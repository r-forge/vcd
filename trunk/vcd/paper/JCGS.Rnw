...
\begin{figure}[htbp]
\begin{center}
<<goals-mosaic,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
grid.mosaicplot(goals, gp = gp.max)
@
<<art-mosaic,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
grid.mosaicplot(art, gp = gp.max)
@
\caption{Extended mosaic plots for the Bundesliga and Arthritis data sets.}
\label{fig:assoc}
\end{center}
\end{figure}
...
tabplot(margin.table(ucb, c(2,1,3)), panel = function(x, ...) grid.mosaicplot(x, panel = TRUE, legend = FALSE, labels = FALSE, gp = mygp.max, space = 0.02, ...))
@
