...
and the maximum statistic from (\ref{eq:MaxAbs}) is $M = \Sexpr{stat(art_max)}$
...
The implementation in the \proglang{R} system for statistical computing and graphics
\citep{vcd:R:2005} employs colors from
...
The HSV space originally looks like a cone with black at its
...
in 2-way tables. But, in fact, none of the ideas described are really
...
both the strata and the cells which caused the dependence.
...
are device independent and provide uniform saturations over different hues.
