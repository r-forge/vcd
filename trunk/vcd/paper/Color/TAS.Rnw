...
of HSV and HCL colors. Section~\ref{sec:palettes} introduces HCL-based qualitative,
sequential, and diverging palettes and Section~\ref{sec:illustrations} shows some brief
illustrations for each type of palette in statistical graphics. Section~\ref{sec:software}
provides some general remarks on the implementation in statistical software packages
as well as some details on our implementation in \proglang{R}. Section~\ref{sec:discussion}
...
if(generate_png) {
  ...
  png(file = PNG(files[1]), height = 400, width = 600)
  hcl.wheel(luminance = seq(5, 95, length = 6))
  dev.off()
  png(file = PNG(files[2]), height = 400, width = 600)
...
has the shape of a distorted double cone: the admissible combinations of
...
a gray scale as in Figure~\ref{fig:hsv-hcl-gray}. This balancing of HCL colors
...
because the admissible values of in the chrome/luminance plane differ across hues.
...
used in Figure~\ref{fig:pal-s-construct} were chosen because they correspond to similar
...
distinguish for color-blind people.
...
to statistical graphics. The first examples illustrates qualitative palettes
and visualizes data from the 2005 election for the German parliament ``Bundestag''
are employed. In this election, five parties were able to obtain enough votes to enter the
...
\begin{figure}[p]
\begin{center}
<<votes, fig=TRUE, height=7, width=8.6>>=
data("Bundestag2005")
...
Faithful geyser eruptions data \citep{vcd:Azzalini+Bowman:1990} is visualized.
...
of a geyser eruption and the waiting time until the next eruption.
Figure~\ref{fig:bkde} shows (two versions of) a heatmap of a bivariate kernel density estimate of 
duration and waiting time revealing a multi-modal bivariate distribution with more
variation in the waiting time after a longer eruption. Both versions of the heatmap use a sequential
...
if(generate_png) {
  ...
  image(dens$x1, dens$x2, dens$fhat, xlab = "duration", ylab = "waiting", 
    col = rev(sequential_hcl(33, c = 0, l = c(30, 90), power = c(1/5, 1.5))))
  box()
  image(dens$x1, dens$x2, dens$fhat, xlab = "duration", ylab = "waiting", 
    col = rev(heat_hcl(33, c = c(80, 30), l = c(30, 90), power = c(1/5, 1.5))))
  box()
  dev.off()
}
@
...
Figure~\ref{fig:class} shows a heatmap with the fit of a support vector machine (SVM) to an artificial 
2-class supervised learning example: a mixture of two bivariate normal distributions with
...
Both diverging palettes employ a smaller contrast on the luminance axis ($L \in [30, 90]$)
and a larger chroma contrast ($C \in [0, 100]$) compared to the sequential palettes from
the previous example. For Figure~\ref{fig:class}, we set $p_1$ and $p_2$ to the somewhat
smaller value of $1.3$ to bring out all deviations from the decision boundary and not only the
extreme decision values. For Figure~\ref{fig:arthritis}, the choice of
$p_1$ and $p_2$ is not so important because only 5 colors are selected---we use values
$p_1 = p_2 = 1$.
...
if(generate_png) {
  ...
  plot(fm, data = ex1)
  ## uses: diverge_hcl(n, c = c(100, 0), l = c(50, 90), power = 1.3)
  dev.off()
}
@
...
palette can easily be computed. A bit more work is required if the sofware
...
implement and can for example be found in \cite{vcd:Wiki+HSV:2006} or
...
\pkg{colorspace} \citep{vcd:Ihaka:2004} which we build upon for implementing all
...
provided by the \pkg{colorspace} package. 
...
experience (as illustrated in Section~\ref{sec:illustrations}, chroma contrasts
...
\code{hcl(h, c, l, ...)} function proveded by the \pkg{colorspace} package
