...
how to choose an appropriate palette for a certain visualization task. Here, we address
...
an online tool for selecting color palettes for maps, providing sets of colors
for coding various types of information. Here, we take a similar approach
...
Unlike \pkg{ColorBrewer.org}, we do not only provide fixed sets of colors (with a limited
number of colors in each set) but describe paths through a perceptually-based color space
so that the user can decide which path exactly should be taken and how many colors should
be selected. The color space employed by us is the Hue-Chroma-Luminance (HCL) color space
\citep[see][]{vcd:Ihaka:2003} and we derive explicit formulae describing the paths through
this space that correspond to different types of palettes. Given a mapping from HCL to RGB
coordinates, our formulas are extremely easy to implement and we provide such an implementation
in the \proglang{R} language \citep{vcd:R:2006} using the powerful \proglang{R} graphics
system \citep[see][]{vcd:Murrell:2006}. A flexible implementation of all palettes described
is provided in the package \pkg{vcd} \citep{vcd:Meyer+Zeileis+Hornik:2006} using the HCL
color implementation from \pkg{colorspace} \citep{vcd:Ihaka:2004}. This gives the user
both a conceptual and computational tool box for experimenting with color palettes for
a particular display.

The remainder of the paper is organized as follows: Section~\ref{sec:spaces} provides
further information on HCL and other color spaces, contrasting in particular the properties
of HSV and HCL colors. Sections~\ref{sec:qualitative}, \ref{sec:sequential}, and \ref{sec:diverging}
introduce HCL-based qualitative, sequential, and diverging palettes, respectively.
Section~\ref{sec:illustrations} shows some brief illustrations for each type of palette
in statistical graphics before Section~\ref{sec:discussion} concludes the paper with a
discussion. Some supplementary \proglang{R} code that facilitates experimentation with
the new palettes is provided in Appendix~\ref{sec:R}.
...
\caption{\label{fig:hsv-hcl} HSV and HCL space. The circles show the hue/saturation
and hue/chroma plane, respectively, for varying levels of value and luminance.}
\end{center}
\end{figure}
...
\caption{\label{fig:hsv-hcl-gray} HSV and HCL space (in gray levels). The circles
show the hue/saturation and hue/chroma plane, respectively, for varying levels of
value and luminance.}
\end{center}
\end{figure}
...
statistics software packages, are \emph{HSV} (hue, saturation, value) colors.
They are a simple transformation of \emph{RGB} (red, green, blue) colors
...
CIELUV, \emph{HCL} (hue, chroma, luminance) colors are obtained, defined by
...
\end{Soutput}
wher \code{n} controls the number of colors in the palette. The arguments
\code{c} and \code{l} give the fixed chroma and luminance level, respectively,
...
and transforms them to RGB coordinates by calling the \code{hcl()} function 
provided by the \pkg{colorspace} package. The \proglang{R} code for reproducing
Figure~\ref{fig:pal-q-examples} is given in the appendix.
...
most basic perceptual axis. The interestingness is thus coded
...
(by default light gray). The \code{power} argument implements $p$ and defaults to $1.5$.
...
One application would be a HCL-based version of heat colors that increase from a light yellow
...
\end{Soutput}
with which the lower three rows in Figure~\ref{fig:pal-s-examples} are produced. The
\proglang{R} code reproducing this figure and stating the exact parameters used is again
...
because the admittable values of in the chrome/luminance plane differ across hues.
Clearly, for deriving symmetric palettes, only colors from the intersection of the
admittable chroma/luminance planes can be used. The particular hues $H = 0$ and $260$
used in Figure~\ref{fig:pal-s-construct} were chosen because they have rather similar
chroma/luminance planes, allowing for both large chroma and luminance contrasts.
...
to statistical graphics. The first illustration is a visualization of a bivariate
density estimation for the Old Faithful geyser eruptions data \citep{vcd:Azzalini+Bowman:1990}.
Figure~\ref{fig:bkde}
...
if(generate_png) {
  ...
  image(dens$x1, dens$x2, sqrt(dens$fhat), xlab = "duration", ylab = "waiting", 
    col = rev(sequential_hcl(33, c = 0, l = c(30, 90), power = c(1/5, 2))))
  box()
  image(dens$x1, dens$x2, sqrt(dens$fhat), xlab = "duration", ylab = "waiting", 
...
parties <- parties[colnames(votes)]
dimnames(votes)[[2]][c(4, 5)] <- c("Gr", "Li")
mosaic(votes, gp = gpar(fill = parties),
...
and Gruene nor the opposition of CDU/CSU and FDP could assemble a majority. Given that no party
would enter a coalition with the leftists, this lead to a big coalition of CDU/CSU and SPD.
...
This gives the users the possibility to easily experiment with the HCL-based
palettes by varying several degrees of freedom. For qualitative
...
experience, chroma contrasts work sufficiently well if a small set of colors
is used. However, when a larger set of colors is used (e.g., for heatmaps
where extreme values should be identifiable) it is much more important to
have a big difference in luminance. Another degree of freedom in these 
palettes is the rate at which the intensity $|i|$ is increased from $0$ to $1$:
Small powers $p$ can be used if palettes with a lot of color should be 
...
provided in the \proglang{R} system (and easily implemented in other
statistical software packages), users can generate palettes varying these
degrees of freedom and thus adapting the colors to their particular graphical display.
...
\code{hcl(h, c, l, ...)} function provedid by the \pkg{colorspace} package
\citep{vcd:Ihaka:2004}. A simple convenience function for displaying a certain palette
is
\begin{Soutput}
...
Further information on all palettes is available from the corresponding
online documentation. Examples that compare the new HCL-based palettes with
those available in base \proglang{R} can be generated via \code{example("rainbow_hcl")}.
