...
oldest and simples perceptual axis. The interestingness is thus coded
...
controls wether intensity/luminance is increased quickly or not. We found $f(i) = i^p$ 
...
votes <- votes[c(1, 3, 4, 5, 9, 11, 13, 14, 15, 16, 2, 6, 7, 8, 10, 12),]
votes <- votes[,c(2, 4, 1, 3, 5)]
votes <- as.table(as.matrix(votes))
...
social democrats SPD, a blue hue $H = 240$ for the conservative CDU, a yellow hue $H = 60$
...
be found in \citep{vcd:Zeileis+Meyer+Hornik:2005}.
