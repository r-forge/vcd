...
%% Hyperref
\hypersetup{%
  pdftitle = {Visualizing Multi-way Contingency Tables},
...
uc_max <- coindep_test(aperm(UCBAdmissions, c(3,2,1)), margin = "Dept", n = 5000)
...
patterns for ordinal variables. However, for nominal variables, mosaic plots are
a better choice, especially when residual-based 
shadings are used (see next section), Association plots directly
...
described in Sect.~\ref{sec:colors} by
...
admission differs between male and female students, and also varies given department.
...
@
\caption{Mosaic plot with highlighting for the \data{UCBAdmissions} data.}
...
doubledecker(aperm(UCBAdmissions), col = grey.colors(2))
@
\caption{Dobledecker plot for the \data{UCBAdmissions} data.}
\label{fig:doubleucb}
\end{center}
\end{figure}
...
shade = TRUE, keep = FALSE, residuals = uc_max$residuals,
gp_args = list(p.value = uc_max$p.value)) 
@
\caption{Mosaic plot for the \data{UCBAdmissions} data.}
\label{fig:ucbmosaic}
\end{center}
\end{figure}
...
residuals_type = "Pearson", residuals = uc_max$residuals,
gp_args = list(p.value = uc_max$p.value))
@
\caption{Association plot for the \data{UCBAdmissions} data.}
\label{fig:ucbassoc}
\end{center}
\end{figure}
...
@
\caption{Conditional mosaic plot for the \data{UCBAdmissions} data.}
...
indicating whether a person has no or moderate attitude towards punishment of children.
...
\hline
\end{tabular}
\caption{The \data{Punishment} data, in flat representation.}
\label{tab:punish}
\end{center}
\end{table}
