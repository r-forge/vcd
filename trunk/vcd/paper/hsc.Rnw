...
spaces. Apart from esthetic considerations, wrongly chosen colors
...
\item Lighter colors tend to make areas look larger than darker
  colors, therefore using colors with unequal luminance values  make it
...
\item Color palettes derived from
  ...
  their colorfulness or lightness. When tiles are shaded using such a
...
\noindent Implementations are often based on so-called
...
represented by three dimensions: the hue (dominant wavelength in the spectrum), 
the saturation (`colorfulness' or `pureness'), and 
the value (`brightness', amount of gray). These intuitive dimensions
...
palette in the HSV space with colors ranging from a saturated red over a neutral white
to a saturated blue. Although the palette should be balanced with
respect to colorfulness and lightness, the red colors are perceived
...
Chroma loosely corresponds to colorfulness and luminance to
...
chroma and luminance, and dividing the hue range in equally-sized intervals. 
HCL colors with fixed chroma and luminance 
...
Similarly, diverging color palettes can be derived
\cite{vcd:Zeileis+Meyer+Hornik:2005} that are
suited, e.g., for visualizing residuals as we will see in the next
section. In principle, for each of the two bipolar hues of the
diverging palette, we could hold constant 
the luminance and only vary the chroma to get the different colors. 
However, it turns out that the resulting colors 
are too similar to be used for discriminating shaded
objects. Better contrasting effects can be achieved by \emph{simultaneously}
varying chroma and luminance, for example by choosing two points in
the chroma/luminance plane for a given hue and interpolating
linearly. For each half of the diverging palette, 
one of these points should be a ``neutral'' color,
and the other correspond to the ending part of the scale. 
The lower part of Fig.~\ref{fig:diverging} shows a diverging
palette using HCL colors. In contrast to the HSV counterpart, matching
colors are balanced to the same gray, and thus get the same perceptual
``weight''.

\subsection{Using Colors for Residuals}
\label{sec:shadings}

All plots introduced in Section~\ref{sec:twoway} can be improved using colors,
...
data, this time using a Friendly-like color coding (with HCL
...
Figure~\ref{fig:Friendly} includes the $p$ value of a $\chi^2$ test
...
using darker (`uninteresting') colors for non-significant results by
restricting the luminance range used for interpolating the colors.
...
gender (see Tab.~\ref{tab:arthritis}):
...
95\% level ($p=0.0161$): possibly, because too many brown-haired male
...
the gender variable simplifies the distinction between the two
...
very unevenly distributed marginals, the strata can become very distorted.
Another possibility is to use trellis-like layouts for visualizing \emph{partial}
...
hypothesis of conditional independence. The overall deviation pattern indicates that 
in contrast to the impression given by the aggregated table, male
