...
\begin{figure}
\begin{center}
%FIXME% right margin labels get clipped
...
gp_args = list(p.value = ucb_max$p.value)) 
@
...
cotabplot(aperm(UCBAdmissions, c(2,1,3)), panel = cotab_coindep, shade = TRUE, 
          legend = FALSE, margins = c(3,1,1,3), panel_args = list(type = "assoc"))
@
\caption{Conditional mosaic plot for the \data{UCB admissions} data.}
...
\begin{figure}
\begin{center}
<<punishcotab,fig=TRUE,echo=FALSE,height=6,width=10,results=hide>>=
cotabplot(tab, panel = cotab_coindep, panel_args = list(margin = c(2.5, 0, 0.5, 0)))
@
\caption{Conditional mosaic plot for the \data{punishment} data.}
