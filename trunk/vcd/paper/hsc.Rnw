...
\begin{itemize}
  ...
  plots \cite{vcd:riedwyl+schuepbach:1994}, or spinograms \cite{vcd:hofmann+theus}.
...
of a contingency table by recursive conditional splits. If the variables can be
separated into explanatory and dependent variable(s), the explanatory variable(s)
should be be used first for splitting to display the conditional distribution of
the dependent variable(s) given the explanatory variable(s). 
...
independence models is of primary interest.
...
In Fig.~\ref{fig:Friendly}, we show again the mosaic for the \data{hospital}
...
In Fig.~\ref{fig:arthritis2}, we visualize again the \data{arthritis} data, this
time using the maximum test statistic and its 10\% and 1% critical values at cut-off points.
The shading of the
...
In Fig.~\ref{fig:pairs}, the diagonal cells show bar plots for the
...
department and highlighting admission in the tiles (see Fig.~\ref{fig:doubleucb}).
...
in Figs.~\ref{fig:ucbmosaic}--\ref{fig:ucbassoc}. The shading visualizes the 
...
The data consists of four variables: Attitude is a binary variable
indicating whether a person approves no or moderate punishment of children.
Memory indicates whether the person has got memories of 
corporal punishment as a child. Education indicates the
...
We first create a mosaic plot to get an overview on the data:
...
highlighted in the tiles (see Fig.~\ref{fig:punishhigh}).
We can see that half of the people have more than 40 years of
age. Most people in this group have only completed elementay
school. For people over 25, a majority has completed secondary
school. Among the younger people, fewer have memories of having been
punished as a child than among the older ones. But sad enough, 
for people with elementary or secondary school education,
a higher proportion \emph{with} memories tends to
have a moderate attitude to punishment than those without.
However, for people under 40, the proportion with
no attitude to punishment increases with the educational level.
...
of memory on attitude to punishment apart from that? Or, rephrased as a hypothesis:
...
plots (see Fig.~\ref{fig:punishcotab}) for the strata defined by all factorial 
...
with elementary school education: under the null, more people should have had
no attitude to physical punishment of children than actually had.
