...
explorative purposes, followed by model-based methods for
conditional independence models.

\subsection{Explorative Visualization Techniques}
\label{sec:flat}
...
A first step in the explorative analysis of more complex tables is to
...
%FIXME% Didn't we decide to use fewer plots in the paper and simply
%FIXME% point out that other combinations could also be used?
%FIXME% I would use for UCB:
%FIXME%   o explorative: highlighted doubledecker (currently Fig. 17)
%FIXME%   o model-based: conidtional shaded assoc (currently Fig. 20)
%FIXME% For Punishment:
%FIXME%   o explorative: highlighted mosaic( currently Fig. 21)
%FIXME%   o model-based: conditional shaded mosaic (currently Fig. 23)
%FIXME% The others should be omitted (I think) and the space gained should
%FIXME% be used for making the individual graphs larger. Preferably, we
%FIXME% should have the explorative and model-based plots for the same data
%FIXME% set on the same page, emphasizing that different aspects of the data
%FIXME% are brought out by the graphs.
...
A natural way of representing such a three-way table is 
to draw a doubledecker plot nesting gender in
department and highlighting admission in the tiles (see Figure~\ref{fig:doubleucb}).
From the widths of the tiles it can be seen that an
unequal number of students apply for the six
departments, and from the highlighting we see that the admission rates differ as
well among the departments (roughly speaking, they decrease from A to F). The rates 
...
In addition to the explorative approaches 
...
A first approach is to use pairs plots for model search:
...
In our example, from the explorative analysis, we already suspect that
...
in Figures~\ref{fig:ucbmosaic}--\ref{fig:ucbassoc}. The shading visualizes the 
...
However, using a single plot, the strata are not corrected for the
marginal distribution of the stratifying variables. Especially for
...
In addition, trellis layouts help reduce the complexity of bigger tables. 
Figure~\ref{fig:ucbadmissions} 
...
We first create a mosaic plot to get an overview of the data:
Attitude towards punishment clearly is the dependent variable 
here and should be used last for splitting. Furthermore, we might 
expect age and education to have stratifying influence on the results, so they should
be used first for splitting.
In fact, the question is whether memory has some influence on attitude, given 
...
age. Most people in this group have only completed elementary
school. For people over 25, a majority has completed secondary
school. 
%% FIXME: additional plot?
Among the younger people, fewer have memories of having been
punished as a child than among the older ones. 
%%
But surprisingly enough,
for people with elementary or secondary school education,
...
However, for people under 40, the proportion with
people refusing punishment increases with the educational level.
...
So both age and education seem to matter here. But is there really any influence
of memory on the attitude towards 
...
We can visualize the model by means of a trellis layout, this time using partial mosaic
plots (see Figure~\ref{fig:punishcotab}) for the strata defined by all factorial 
combinations of the age and education levels. In each panel, we plot mosaic plots of 
memory and attitude, where again the explanatory variable (memory) is used first 
for splitting.
The shading tells us the model seems to be consistent with the data, 
except for people over 25
with elementary school education: under the null, more people should
have refused physical punishment of children than actually did.
...
strongly influenced by the marginal distribution of education over age (in particular
 ...
 conditional mosaic plot suppresses the effect of all stratifying
...
cotabplot(tab, panel = cotab_coindep, panel_args = list(varnames = FALSE, 
margins = c(2, 1, 1, 2)))
@
...
for exploratory model search (pairs plots, highlighting, doubledecker plots) 
and stratified analysis (conditional plots).
