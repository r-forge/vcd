...
\begin{table}[h]
  ...
  \label{tab:compare}
\end{table}
%FIXME% For the JSS paper there was criticism about haven MANET and Mondrian in the
%FIXME% same column, how did we decide ro resolve this? Different columns, I suppose...?!
...
%FIXME% causality is not needed, hence omitted: in (hypothesized) causal relationships.
...
%FIXME% Maybe formulate the following theory a bit more like:
%FIXME% "To fix notations, ...", "It is well-known that..."
...
the dependent variable(s) given the explanatory variable(s). %FIXME% causality removed again
...
\subsection[A Note on Colors and Color Palettes]{A Note on Colors and Color Palettes\footnote{The electronic version of this article uses colors.}}
\label{sec:colors} 
%FIXME% Maybe extend the footnote and say that the printed version is monochrome,
%FIXME% and that the electronic version uses color.
%FIXME% We can already say earlier that the R code for reproducing the figures
%FIXME% is available from a Web page. And then we can say here that the colored versions
%FIXME% can also be obtained from the Web page.
%FIXME% Where do we want to set up the URL? I would suggest
%FIXME%   http://statmath.wu-wien.ac.at/projects/vcd/
...
A possible way of representing such a three-way table is 
to draw a mosaic plot of gender and
department and to highlight admission in the tiles (see Fig.~\ref{fig:ucbhighlight}).
...
well among the departments. The rates 
are equal for male and female students, except for department A where
\textit{more} female than male students are admitted. This result
is even better depicted by a doubledecker plot of admission, explained
by gender given department (see Fig.~\ref{fig:doubleucb}): here, the
difference of admission rates in department A is more marked.
...
@
\caption{Dobledecker plot for the \data{UCB admissions} data.}
\label{fig:doubleucb}
\end{center}
\end{figure}
...
Mosaic displays are recognized as an excellent
...
To get an overview, we first plot a mosicplot of memory, age, and
education, and highlight attitude (see Fig.~\ref{fig:punishhigh}).
...
So both age and education seem to matter here. Is there any influence
of memory on attitude to punishment apart from that? To answer this
question, we test the hypothesis of memory and attitude being
conditionally independent, given age and education, and visualize the
model using an extended mosaic plot (see Fig.~\ref{fig:punishmosaic}). 
We can see that the model fits rather good, except for people over 25
with elementary school education: under the null, more people should
say ``no'' to physical punishment of children. This pattern can again also
be visualized using a trellis layout, this time using partial mosaic
plots (see Fig.~\ref{fig:punishmosaic}).
