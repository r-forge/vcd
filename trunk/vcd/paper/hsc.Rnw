...
set.seed(myseed)
ucb_max <- coindep_test(aperm(UCBAdmissions, c(3,2,1)), margin = "Dept", n = 5000)
...
(Figure~\ref{fig:threedee} only), all freely available from the
...
the \data{hospital} data \cite{vcd:wing:1962} given in Table~\ref{tab:hospital}.
...
grouped bar plots (see Figure~\ref{fig:grouped}) 
or even by means of 3D-bar charts (see Figure~\ref{fig:threedee}). It seems hard
...
(see Figure~\ref{fig:sievefig}). It implicitly compares
...
most widely used residuals are the Pearson residuals
...
and hence software implementations are often based on
...
admit---nested in gender---into the rows, is given by
...
arranged in a matrix similar to scatterplots in a pairs plot.
...
doubledecker plot in Figure~\ref{fig:doubleucb}. From the widths of the
...
\begin{figure}
\begin{center}
<<doubleucb,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
model of admission and gender being conditionally independent, given department. Clearly,
there is no gender bias in the departments except for department A: here, 
...
gp_args = list(p.value = ucb_max$p.value), rep = c(Admit = FALSE)) 
@
...
%FIXME%   o dependent variable: attitude
%FIXME%   o dependence on age and education is rather clear
%FIXME%     (approval of moderate punishment increases with age and
%FIXME%     decreases with education)
%FIXME%   o question is conditional independence of attitude and
%FIXME%     memory given the other margins
%FIXME%   o hence for exploratory analysis: split in conditioning
%FIXME%     variables (age, education) first, then explanatory variable (memory),
%FIXME%     then dependent variable (attitude) for which highlighting is used.
%FIXME%   o for model-based view: suppress marginal distribution of
%FIXME%     conditioning variables (and their interaction), split first
%FIXME%     in explanatory variable in each cell, use shading based on
%FIXME%     residuals in conditional independence model.
%FIXME%   o advantage of exploratory view: joint distribution of all variables
%FIXME%     can be seen. disadvantage: visualization strongly influence
%FIXME%     by the marginal distribution of education over age (in particular
%FIXME%     the large proportion of 40+ people with elementary education)
%FIXME%     which is not of interest in the conditional independence problem.
...
% ##        gp_args = list(interpolate = tab_test$qdist(c(0.9, 0.99))), 
...
set.seed(123)
cotabplot(tab, panel = cotab_coindep, panel_args = list(varnames = FALSE, margins = c(2, 1, 1, 2)))
@
