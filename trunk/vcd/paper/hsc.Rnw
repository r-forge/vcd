...
objects. Better contrasting can be achieved by \emph{simultaneously}
...
are balanced to the same gray, and thus get the same perceptual ``weight''.

\subsection{Highlighting and Color-based Shadings}
\label{sec:shadings}

All plots introduced in Sect.~\ref{sec:twoway} are composed of empty
tiles. It seems intuitive to used filled tiles to highlight information
...
mosaic(t(hospital), 
       ...
       gp = gpar(fill = c("gray","black","black")))
@ 
