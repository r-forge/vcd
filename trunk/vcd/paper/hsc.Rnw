...
the \data{hospital} data \cite{vcd:wing:1962} given in Tab.~\ref{tab:hospital}.
...
\subsection[A Note on Colors and Color Palettes]{A Note on Colors and
  Color Palettes\footnote{The printed version of the article is
    ...
    versions of the plots are available from the code web page 
...
described in Sect.~\ref{sec:colors} but using a smaller amount of color, i.e.,
...
analysis. Highlighting of tiles can support the analysis relationships with a 
...
In Sect.~\ref{sec:twoway}, we have presented basic displays
...
gender. The `flattened' contingency table, putting department into the
columns and admit---nested in gender---into the rows, is given by
Tab.~\ref{tab:ucb}. The table aggregated over all
departments gives wrong evidence of gender bias, 
i.e., higher admission rates for male students. 
...
get a quick overview on the data. For that, 
...
A natural way of representing such a three-way table is use a mosaic display
...
doubledecker plot in Figure~\ref{fig:doubleucb}. From the widths of the tiles it can be seen that an
unequal number of students apply for the six departments (with a particularly small
...
A first approach is to use pairs plots (such as that in Figure~\ref{fig:pairs}) for model search:
...
nest admission into department instead of gender to get a stratified view.
...
data by using the conditioning variables first for splitting.
However, this also displays the marginal distribution of the conditioning variables
...
punishment of children (see Tab.~\ref{tab:punish}).      
...
To focus on this question, we will take another model-based view on the data
that avoids the distracting influence of the marginal variables (age
