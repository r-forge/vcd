...
\begin{table}[h]
  ...
  \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|}
    ...
    \proglang{S} & \proglang{S} & \proglang{Java} & \proglang{XLisp} & \proglang{C++} & \proglang{Java}\\
\hline
  \end{tabular}
...
of independence rejects the null hypothesis: possibly, because
the tendency of people  to have extramarital
sex when they had premarital sex 
is particularly marked among married people?
...
Titanic data again, this time collapsed over `Survived' to investigate the
...
this plot also visualizes the marginal distribution of \code{Sex}.}
...
by position with the columns and by shading with the rows.
...
all dimensions, this simplifies to:
