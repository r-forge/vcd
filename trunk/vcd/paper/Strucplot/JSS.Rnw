...
\pkg{vcd} \citep{vcd:Meyer+Zeileis+Hornik:2006},
...
Most environments are available on Windows, Apple Macintosh, and Linux/Unix
variants, except \proglang{MANET} which is only available for the
Macinthosh platforms. \proglang{Mondrian} is a JAVA application, as
well as major parts of the \pkg{iPlots} package.
...
%%DM%% Fixme: separate columns for iplot, base, and vcd?

Figures \ref{fig:arthritis} to \ref{fig:titanic} illustrate some of these
extensions (the code is given in Appendix \ref{sec:code}). 
...
case, as well as pairs and trellis plot-like displays for
...
modularized by means of `graphical appearance functions', allowing
...
before Section \ref{sec:conclusion} concludes the work.
...
These are examples of `\underline{gr}aphical \underline{ap}pearance \underline{con}trol'
  ...
  \begin{tabular}{|l|l|l|}
    ...
    core      & \codefun{struc\_mosaic} & core function for mosaic plots\\
     ...
     shading  & \codefun{shading\_binary} & visualizes the sign of the  residuals\\
              & \codefun{shading\_Friendly} & implements Friendly-like
              shading (based on HSV-colors)\\
              & \codefun{shading\_sieve} & implements Friendly-like
              shading used for sieve plots\\ && (based on HCL-colors)\\
              ...
              maximum test statistic\\ && (based on HCL-colors)\\\hline\hline
...
\noindent For zero entries, tiles have zero area and are, additionally,
marked by small bullets (see, e.g, Figure \ref{fig:doubledecker}). By
default, these cells are not splitted further. The bullets help
...
\codefun{doubledecker.default} is simply a wrapper for 
\codefun{strucplot}, setting the right defaults:

<<doubledecker>>=
doubledecker.default <- 
  function(x,
           depvar = length(dim(x)), 
...
\noindent Most default settings such as colors, spacing, and labeling are specified via the parameters of
\codefun{doubledecker.default} and passed through to
\codefun{strucplot}. The additional code just handles the dependent
variable information, and in particular permutes the table to have the
...
on blocks defined by the variable levels. In the following, we again use the
Titanic data, this time collapsed over `Survived' to investigate the
structure of crew and passengers:

<<subsetting,results=verbatim>>=
(tit <- structable(~ Sex + Class + Age, data = Titanic[,,2:1,]))
tit["Male",]
tit["Male", c("1st","2nd","3rd")]
@ 
...
mosaic(tit[["Male"]], margins = c(left = 2.5, top = 2.5, 0), sub = "Male", newpage = FALSE)
popViewport()
@ 

<<Variables3>>=
pushViewport(viewport(layout.pos.col = 2))
mosaic(tit[["Female"]], margins = c(top = 2.5, 0), sub = "Female", newpage = FALSE)
popViewport(2)
@
...
<<cotabplot>>=
cotabplot(~ Class + Age | Sex, data = tit, split_vertical = TRUE)
@
...
conditioning variable(s) which may or may not be appropriated or
...
<<Conditioning1>>=
mosaic(tit, conditional = "Sex", split_vertical = c(TRUE, TRUE, FALSE), 
       keep_aspect_ratio = FALSE)
@ 
<<Conditioning2>>=
mosaic(~ Class + Age | Sex, data = tit, split_vertical = c(TRUE, TRUE, FALSE), 
       keep_aspect_ratio = FALSE)
@
...
in the table, and for the formula interface, 
that \codefun{spacing\_conditional} is used as default to better
...
and mosaic plots visualizing mutual independence in the upper and lower triangles.
...
<<pairs>>=
pairs(tit, highlighting = 2, diag_panel = pairs_diagonal_mosaic, 
      diag_panel_args = list(fill = grey.colors, alternate_labels = FALSE))
@
...
\begin{center}
\code{[Optional prefix]cell:\emph{Variable1}=\emph{Level1},\emph{Variable2}=\emph{Level2}} \dots
\end{center}
...
the core functions are the follwing:

\begin{enumerate}
\item recursively, create a tree of nested viewports
\item push the whole tree, i.e., add it to the system tree maintained
  by the \pkg{grid} engine
\item one by one, seek all cells of the strucplot display and add
  plot-specific content (rectangles for mosaics, bars for association
  plots, etc.)
\end{enumerate}
...
\begin{equation}
  \label{eq:numbervp}
  T_d = k_1 + k_1k_2 + \dots + k_1 \dots k_d = \sum_{i=1}^d \prod_{j \le i} k_j
\end{equation}

\noindent since we first push the $k_1$ viewports for the levels of the
first dimension, then, for \emph{each} of those, the $k_2$ levels of
the second dimension, etc. Assuming an equal number of levels $k$
for all dimensions, we obtain:

\begin{equation}
  \label{eq:equalvp}
  T_d = \sum_{i=1}^d k^i = \frac{k(k^d-1)}{k-1}
\end{equation}
...
Color palettes in HCL space are trickier
to choose, but preferable to palettes derived 
from HSV space from a perceptual point of view.
...
\noindent (Note that the bullets drawn for zero cells are shaded, too). 
...
this time with HCL colors (Figure \ref{fig:hclshading1} depicts the default palette, 
...
the strucplot framework, it suffices to set the space between the `Improved' tiles to 0 (see
...
\section{Example: the Ovarian Cancer Data}
\label{sec:example}

In the following, we demonstrate some of the described techniques in
analyzing the Ovarian Cancer data set.
The data origins from \citep{vcd:obel:1975} and is 
about a retrospective study of ovary cancer
...
four binary variables: the `stage' of the cancer at the time of
operation (levels: early, advanced), the type of `operation' performed
(radical, limited), the `survival' status after 10 years (yes, no),
and `xray' indicating whether X-ray treatment was received (yes,
no). 
...
were treated with a radical operation, and 186 (62\%) got X-ray treatment. Next, we inspect
the two-way interaction of the influencing factors (`stage', `operation', and `xray'): the
...
The same is true for the interactions of `survival'
with `operation' and `xray', respectively. Only the stage seems to influence
survival: here, the tiles are ``shifted''.

A different view on the data can be obtained using a doubledecker plot, explaining
survival by the three other factors (Figure \ref{fig:ocdoubledecker}):
...
\begin{figure}[h]
\begin{center}
<<ocdoubledecker,echo=FALSE,fig=TRUE>>=
<<oc4>>
@ 
\caption{Doubledecker plot for the \data{OvaryCancer} data showing the
 conditional distribution of `xray', given `operation', given `stage', and with
...
showing the conditional distribution of `survival', given `xray', given `operation', 
given `stage', with vertical splits for the conditioning variables and horizontal ones for
`survival'. Additionally, there is zero space between the tiles of the last dimension and
...
variables, with `survival' highlighted in the tiles. Thus, the plot really shows
the influence of the ``explanatory'' variables on `survival'. Clearly, the survival
...
phase, it even seems that for patients in an
early stage with a limited operation, no X-ray treatment would be better, 
and that for patients in an advanced stage, limited in contrast to radical 
operation would improve the chance of survival. %'

To visualize inference results, we can make use of residual-based shadings. First, we
again use the pairs plot display, this time using color to bring out departures from
the hypothesized marginal independence models (Figure \ref{fig:ocpairsshaded}):

<<oc5>>=
pairs(tab, diag_panel_args = dpa, gp = shading_max)
@ 

\begin{figure}[h]
\begin{center}
<<ocpairsshaded,echo=FALSE,fig=TRUE>>=
<<oc5>>
@ 
\caption{Pairs plot for the \data{OvaryCancer} data showing mosaic displays for all pairwise distributions and bar plots for all marginal distributions. The colored cells show the
violation of the independence hypothesis for `stage' and `survival'.}
\label{fig:ocpairsshaded}
\end{center}
\end{figure}

\noindent Only the mosaics showing the interaction of `stage' and `survival' have 
colored cells, indicating that the survival rate of patients in an early stage is
higher than would be expected under (marginal) independence. The hypotheses of
independence are not rejected for all other marginal interactions. Next, we investigate
log-linear models for the four-way table. The default model used by \codefun{mosaic} is
the total independence model, visualized in Figure \ref{fig:ocmosaictotal}:
...
\begin{figure}[h]
\begin{center}
<<ocmosaictotal,echo=FALSE,fig=TRUE>>=
<<oc6>>
@ 
\caption{Mosaic plot for the \data{OvaryCancer} data, with residual-based shading
for the (clearly rejected) default hypothesis of total independence of all variables.}
\label{fig:ocmosaictotal}
\end{center}
\end{figure}

\noindent The model is clearly rejected (P-value: 0.000). From the exploratory phase of
our analysis, we (only) suspect `stage' to be influential on the survival rate. A 
corresponding hypothesis is that `survival' be independent of `xray' and `operation', given
`stage'. The model is specified using the \texttt{expected} argument, 
...
\begin{figure}[h]
\begin{center}
<<ocmosaicstage,echo=FALSE,fig=TRUE>>=
<<oc7>>
@ 
\caption{Mosaic plot for the \data{OvaryCancer} data, with residual-based shading
for the hypothesis of `survival' being independent of `xray' and `operation', given
...
\item[\texttt{OvaryCancer}]  Data about a retrospective study of ovary cancer
     ...
     \cite{vcd:obel:1975}. Package: \pkg{vcd}.
...
\item Mosaic plot for the \data{Arthritis} data in Figure \ref{fig:arthritis}:
\begin{Sinput}
mosaic(art, gp = shading_max, split_vertical = TRUE)
\end{Sinput}
