...
\begin{equation}
  \label{eq:numbervp}
  T_d = \sum_{i=1}_d \prod_{j \le i} k_j
\end{equation}

\noindent Assuming an equal number for all dimensions, we obtain:

\begin{equation}
  \label{eq:equalvp}
  T_d = \sum_{i=1}_d k^i = \frac{k(k^d-1)}{k-1}
\end{equation}

\noindent and so the time complexity for drawing a strucplot display is of order $O(k^d)$.
