...
(tit <- structable(~ Sex + Class + Age, data = Titanic))
tit["Male",]
...
<<Conditioning>>=
mosaic(~ Age + Class | Sex, data = tit, split_vertical = TRUE, keep_aspect_ratio = FALSE)
@
...
<<cotabplot>>=
cotabplot(~ Age + Class | Sex, data = tit, panel_args = list(margins = 3), 
          labeling = labeling_left, tl_labels = c(Class = FALSE))
@
...
\data{HairEyeColor} data with univariate mosaics in the diagonal,
...
% auch möglich mit Titanic-Daten:
% <<pairs>>=
