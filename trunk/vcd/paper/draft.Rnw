...
By employing this approach the permutation distribution of statistics of the
type $\lambda(r_{ij})$, i.e., also of the $\chi^2$ statistic, can be derived.

Other classical tests for the hypothesis of independence  which do not fit in
the Pearson residual-based framework described aboveinclude Fisher's exact test,
...
coded by position relative to the baseline and color of the corresponding rectangle.
...
the expected frequencies under independence (which does not only work in 2-way
tables but also log-linear models for multi-way tables). In this
extension, positive and negative signs of the residuals are coded by solid and dashed line
rectangles respectively. Furthermore, residuals exceeding an absolute
...
behind this shading is that as
the Pearson residuals are approximately $\mathcal{N}(0,1)$ distributed
values, which implies that the highlighted cells are those with
residuals individually significant at approximately the 5\% and 0.01\%
level. But the main purpose of the shading is to visualize the pattern of
deviation from independence \citep{vcd:Friendly:2000}. In particular,
it does not provide a visualization of the maximum test (\ref{eq:MaxAbs})
or in other words: it is in general unknown to which significance level $\alpha$ the
...
used in the original \textsf{SAS} implementation and also in the base implementation
...
The original implementation of mosaic plots in \textsf{SAS}
used (by default) colors in the HLS (Hue-Luminance-Saturation) space
with hues for blue and red, full saturation and varying luminance
for the lighter colors. The current \textsf{R} implementation is
based on colors in the HSV (Hue-Saturation-Value) space where
decreasing the saturation from 1 towards 0---given the hue for
blue or red and full saturation---generates very similar colors
as increasing the luminance for HSL colors from 0.5 towars 1.
...
used also providing homogeneous saturations over different colors and copier
proofness. The resulting association plot (using a critical values for
...
for the well-known Hair-Eye-Color data set which cross-tabulates the hair and eye
...
it can be seen that the residual corresponding to the black/blue
cell is also significant at the 10\% level.
...
not independent. The HSV space on the other hand has the advantage that all
...
grid.mosaicplot(t(goals), main = NULL)
@
<<haireye-mosaic-signif,fig=TRUE,echo=FALSE,height=6,width=6>>=
grid.mosaicplot(t(haireye), main = NULL)
@
