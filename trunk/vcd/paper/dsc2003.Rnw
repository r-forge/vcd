...
\texttt{grid} \cite{vcd:Murrell:2002}. The new functionality is provided by the {\sf R} package
...
(produced by the implementation in \texttt{base} {\sf R}) for the
...
We propose to use a color-shading similar to
...
this extension 
uses a color coding of the mosaic tiles to visualize deviations
...
$p$ value approximately $0.1013$. Apparently, the relative size of the
...
California at Berkley (UCB)---see \cite{vcd:Bickel+Hammel+O'connell:1975}---which is available in base \textsf{R}.
