...
(residuals) from a given log-linear model fitted to the table.
...
deviation from independence \citep[p.~109]{vcd:Friendly:2000}. A different
approach for visualizing the residuals of log-linear models is
used by \cite{vcd:Theus+Laurer:1999} who propose to use relative
residuals, i.e., to scale all residuals according to the absolute sum
of residuals. This is necessary when shading
cells in a mosaic plot for expected frequencies, but offers no
advantages when the residuals themselves are visualized. 
...
grid.assocplot(t(margin.table(HairEyeColor, c(1,3))), gp = gp.shading)
@
...
like those described in the previous sections.
...
David Meyer\\
Achim Zeileis\\
E-mail: \texttt{\{David.Meyer,Achim.Zeileis\}@ci.tuwien.ac.at}\\
...
\\
Kurt Hornik\\
E-mail: \texttt{Kurt.Hornik@wu-wien.ac.at}\\
