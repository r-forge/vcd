...
the expected frequencies under independence.
...
The big tiles for brown Eyes suggest convey the impression of an asymmetry between
...
because simply no tile is shaded. In fact, as we can depict from
...
@
\caption{Association plot for `Eye' and `Sex' in the `HairEyeColor' data.}
...
@
\caption{Extended association plot for `Eye' and `Sex' in the `HairEyeColor' data.}
