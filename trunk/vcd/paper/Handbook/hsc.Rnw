...
%FIXME: lookup regular in Wing-paper.
...
mosaic (see Figure~\ref{fig:mosaic3fig}). 
...
explanatory, it should be be used first for splitting: thus, the display shows
the conditional distribution of the dependent variable given the explanatory one.
...
\caption{Qualitative color palette for the HSV (left) 
	 ...
	 the HCL colors $(H, 50, 70)$ for the same hues $H$. Note that
         ...
         wheel will be shaded with the same gray, i.e., appear identical.}
\label{fig:colors}
\end{center}
\end{figure}
...
\caption{Diverging color palettes for the HSV space (upper part) and
  ...
  sides of the HCL colors palette appear identical. }
\label{fig:diverging}
\end{center}
\end{figure}
...
All plots introduced in Section~\ref{sec:twoway} are composed of empty
...
\end{center}
\end{figure}
Clearly, the asymmetry for regular and never visited students,
...
In addition, the significance of test statistics can be visualized by using 
...
@
\caption{Doubledecker plot for the \data{UCBAdmissions} data.}
\label{fig:doubleucb}
\end{center}
\end{figure}
...
One approach is employing pairs plots (such as that in Figure~\ref{fig:pairs}) for model search:
...
with similar ideas as in Section~\ref{sec:extensions}. Thus, mosaic displays can
...
Attitude towards punishment clearly is the dependent variable 
...
and to have attitude, the dependent variable, 
...
of people with memories of punishments increases with age and that
...
or model-based analyses (residual-based shading, conditional plots).
