...
\usepackage{Sweave,hyperref,color,graphicx,makeidx,multicol} 
...
\index{Categorical data} Categorical data analysis is typically based on two- or
...
and hence software implementations are often based on \index{HSV colors}
...
showing a qualitative color palette \index{qualitative color palette}
...
\index{Data!Danish corporate punishment}
