...
mosaicplot(HairEyeColor,shade = TRUE, margin = ~Hair*Eye + Sex, main = "Model: Hair*Eye + Sex" )

mosaicplot(HairEyeColor, shade = TRUE, margin = ~Hair*Sex + Eye*Sex, main = "Model: Hair*Sex + Eye*Sex")
...
## (Gender Pre)(Extra) ##
mosaicplot(margin.table(PreSex,c(2,3,4)), shade = TRUE, clegend = FALSE, margin = ~Gender*PremaritalSex + ExtramaritalSex , main = "PreMaritalSex*Gender +Sex")

par(mfrow=c(1,2))

## (Gender Pre Extra)(Marital) ##
mosaicplot(PreSex, shade = TRUE, margin = ~Gender*PremaritalSex*ExtramaritalSex + MaritalStatus, clegend = FALSE, main = "PreMarital*ExtraMarital + MaritalStatus")

## (GPE)(PEM) ##
mosaicplot(PreSex, shade = TRUE, margin = ~Gender*PremaritalSex*ExtramaritalSex + MaritalStatus*PremaritalSex*ExtramaritalSex, clegend = FALSE, main = "G*P*E + P*E*M")
...
## Employment Status ##
mosaicplot(Employment, shade = TRUE, margin = ~LayoffCause*EmploymentLength + EmploymentStatus, main = "Layoff*Employment + EmployStatus")


mosaicplot(Employment, shade = TRUE, margin = ~LayoffCause*EmploymentLength + LayoffCause*EmploymentStatus, main = "Layoff*EmpL + Layoff*EmplS")
