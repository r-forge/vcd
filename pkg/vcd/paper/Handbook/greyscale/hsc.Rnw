...
\usepackage{Sweave,color,hyperref,graphicx,makeidx,multicol}
...
%% Hyperref
\hypersetup{%
  pdftitle = {Visualizing Contingency Tables},
  pdfsubject = {Handbook of Statistical Computing},
  pdfkeywords = {association plots, contingency tables, HCL colors, HSV colors, mosaic plots, sieve plots},
  pdfauthor = {David Meyer, Achim Zeileis, Kurt Hornik},
...
## greyscale-Function
togrey <- function(x) {
  x <- as(hex2RGB(substr(x, 1, 7)), "polarLUV")
  x@coords[,2] <- 0
  hex(x)
}
@
...
\title*{Visualizing Contingency Tables}
...
% your contribution title if the original one is too long
\institute{Department of Information Systems and Operations
...
\index{catgorical data} Categorical data analysis is typically based on two- or
higher-dimensional \index{contingency tables} contingency tables, cross-tabulating the
...
association plots offer good support for visualization. Especially
mosaic and sieve plots have been extended over the last two decades,
and implementations exist in many statistical environments.

All three graphical methods visualize aspects of (possibly higher-dimensional)
contingency tables. A \emph{mosaic plot} \index{mosaic plot}
\cite{vcd:Hartigan+Kleiner:1984} is basically
...
\emph{Sieve plots} \index{sieve plot} \cite{vcd:riedwyl+schuepbach:1994}
...
An \emph{association plot} \index{association plot} \cite{vcd:Cohen:1980}
...
\begin{itemize}
  ...
  double-decker plots \cite{vcd:Hofmann:2001} or spine
  plots \cite{vcd:riedwyl+schuepbach:1994}.
%% or spinograms \cite{vcd:hofmann+theus}.
...
\item Providing a modular and flexible implementation to easily allow
  user extensions \cite{vcd:Meyer+Zeileis+Hornik:2006b}.
\end{itemize}
...
\proglang{R} \cite{vcd:R:2006}, and \proglang{S-PLUS} \cite{vcd:SPLUS:2005}.
...
\proglang{R} \index{R} system, using the extension packages \pkg{vcd}
\cite{vcd:Meyer+Zeileis+Hornik:2006} and
\pkg{scatterplot3d} \cite{vcd:ligges+maechler:2003}
(Figure~\ref{fig:bars3d} only), all freely available from the
Comprehensive \proglang{R} Archive Network
(\url{http://CRAN.R-project.org/}). The \proglang{R} code used for the
figures is available from \url{http://statmath.wu-wien.ac.at/projects/vcd/}.
...
\begin{table}[h]
 ...
 & \proglang{SAS} & \proglang{S-PLUS} & \proglang{R} & \pkg{ViSta} & \pkg{MANET} & \pkg{Mondrian}\\\hline
    Basic functionality      &  $\times$  &   $\times$    & $\times$ &   $\times$   & $\times$& $\times$\\
    Shape                    &     &        & $\times$ &       & $\times$& $\times$\\
    Residual-based shadings  &  $\times$  &        & $\times$ &   $\times$   & ($\times$)& ($\times$)\\
    Conditional Views        &  $\times$  &        & $\times$ &       & $\times$& $\times$\\
    Interaction              &     &        &   &   $\times$   & $\times$& $\times$\\
...
of mosaic and association plots, both for exploratory visualization and
...
In Section~\ref{sec:twoway}, we start with the
...
In Section~\ref{sec:multiway}, we show how the
...
structures. The techniques are illustrated using three- and four-way tables.
Section~\ref{sec:concl} concludes the chapter.

\section{Two-way Tables}
\label{sec:twoway}

Throughout this section, our examples will be based on
the \index{data!hospital}
\data{hospital} data \cite{vcd:wing:1962} given in Table~\ref{tab:hospital}.
...
the frequency of visits (from relatives or friends).
The length of stay (LOS) has been categorized into
2--9 years, 10--19 years, and more than 19 years. There are also three
categories for the visit frequency: regular (including
patients who were allowed to go home), less than monthly, and never.
Wing \cite{vcd:wing:1962} concludes from this data
that the longer the patients stayed in hospital, the less frequent they
are visited, which can be seen from the column-standardized table
(see Table~\ref{tab:colhospital}).
...
strata. From the row-standardized table (see Table~\ref{tab:rowhospital}),
it seems indeed that LOS is homogeneous with respect to these two visit frequency strata.
...
\begin{figure}[p]
\begin{center}
<<barsgrouped,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
\begin{figure}[p]
\begin{center}
<<bars3d,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
grouped bar plots (see Figure~\ref{fig:barsgrouped})
or even by means of 3D-bar charts (see Figure~\ref{fig:bars3d}). It seems hard
...
\index{mosaic plot} Mosaic displays have been introduced by
...
area-proportional tiles, arranged in a rectangular mosaic. The tiles are
obtained by recursive partitioning splits of a rectangle.
In the following, we describe the main idea of mosaic plots;
Chapter III-12 in this book provides more detailed information. Consider our
...
representation, we choose LOS with vertical splits (see Figure~\ref{fig:mosaic1fig}).
...
called a \emph{spine plot} \cite{vcd:hummel:1996}. \index{spine plot}
...
Figure~\ref{fig:mosaic2fig}). The resulting plot visualizes the
...
as the first splitting variable, the impression is very different
compared to the previous mosaic.
In this alternative display (see Figure~\ref{fig:mosaic3fig}),
we see the marginal distribution of `Visit frequency' %'
...
to be the explanatory variable.
...
tables, the two most common and well-known hypotheses \cite{vcd:Agresti:2002} are
...
of these hypotheses. To fix notations, in the following we consider a
...
$n_{i+} = \sum_j n_{ij}$ and $n_{+j} = \sum_i n_{ij}$, respectively. For
...
The expected table for our sample data is given in Table~\ref{tab:hospital2}.
...
(see Figure~\ref{fig:sieve}). It implicitly compares
...
the two variables. This gives evidence for the fact that for these patients,
visit frequency decreases with the length of stay.

\setkeys{Gin}{width=0.7\textwidth}
\begin{figure}
\begin{center}
<<sieve,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
most widely used residuals are the Pearson residuals \index{Pearson residuals}
...
reversed for patients visited less than monthly or never visited.
...
of a contingency table by recursive conditional splits. If one variable is
explanatory, it should be be used first for splitting;
the display then shows the conditional distribution of the
dependent variable given the explanatory one.
Sieve plots basically visualize the table of \emph{expected} frequencies, and
in addition the deviations from the observed frequencies by the density of a
grid added to each tile. They complement mosaic plots in detecting dependency
patterns for ordinal variables. An alternative way of enhancing mosaic plots
to display deviations from expected frequencies is to use
residual-based shadings (see next section) which are typically more intellegible
than sieve plots, in particular for nominal variables.
Association plots directly
...
detection of interesting patterns. We start with some general
remarks on colors and color palettes.

\subsection[A Note on Colors and Color Palettes]{A Note on Colors and
  Color Palettes\footnote{The printed version of the article is
    monochrome, but the electronic version uses colors. The colored
    versions of the plots are available from the Web page
    \url{http://statmath.wu-wien.ac.at/projects/vcd/}.}}
\label{sec:colors}
...
\item Using high-chroma colors for large
  areas tends to produce after-image effects which can be
...
Choosing colors and color palettes in RGB space can be a bit inconvenient
and hence software implementations are often based on \index{color space!HSV}
Hue-Saturation-Value (HSV) colors (or the comparable Hue-Luminance-Satu\-ration
...
colors in many computer packages \cite{vcd:Moretti+Lyons:2002}. Each color in HSV space is
...
make HSV colors easier to specify than RGB colors.
However, HSV colors have several disadvantages.
...
As an example, see Figure~\ref{fig:colors} (left)
showing a qualitative color palette \index{color palette!qualitative}
(colors $(H, 100, 100)$ for varying hues $H$) in the HSV
...
\index{color palette!diverging}
bipolar scales containing colors ranging between two very distinct colors.
The upper part of Figure~\ref{fig:diverging} shows a diverging
...
\begin{figure}
\begin{center}
<<color,fig=TRUE,echo=FALSE,height=3,width=7,results=hide>>=
par(mfrow = c(1,2), mar = c(1,1,1,1), oma = c(0,0,0,0))
pie(rep(1,9), radius = 1, col = togrey(rainbow(9)), labels = 360 * 0:8/9)
pie(rep(1,9), radius = 1, col = togrey(rainbow_hcl(9)), labels = 360 * 0:8/9)
@
\caption{Qualitative color palette for the HSV (left)
         and HCL (right) spaces. The HSV colors are $(H, 100, 100)$ and
	 the HCL colors $(H, 50, 70)$ for the same hues $H$. Note that
         in a monochrome version of this paper, all pies in the right
         wheel will be shaded with the same gray, i.e., they will
...
plot.new()
rect(0:4/5, 0.2, 1:5/5, 0.5, border = 0, col = togrey(diverge_hcl(5, c = 100, l = c(50, 90))))
rect(0, 0.2, 1, 0.5, border = 1, col = NULL)
rect(0:4/5, 0.55, 1:5/5, 0.85, border = 0, col = togrey(diverge_hsv(5)))
...
\caption{Diverging color palettes for the HSV space (upper part) and
  ...
  In a monochrome version of the paper, the right and left hand
  sides of the HCL colors palette appear to be identical, unlike the
  HSV color palette. }
\label{fig:diverging}
\end{center}
\end{figure}
...
\index{color space!CIELAB} \index{color space!CIELUV}
...
Note, that chroma and luminance are varied simultaneously, i.e.,
...
full chroma colors), however, by varying both chroma and luminance better
contrasts can be achieved.

\subsection{Highlighting and Color-based Shadings}
\label{sec:shadings}

The mosaic plots introduced in Section~\ref{sec:twoway} are composed of empty
tiles. It seems intuitive to use filled tiles to highlight information
of interest. Consider again the hospital example: in Figure~\ref{fig:highlight},
...
``stacked'' plot can also be interpreted as spine plot with
highlighting \index{highlighting}
\cite{vcd:hummel:1996}, particularly useful in analyzing relationships among
categorical data with a binary dependent variable. More variations on
this theme, such as doubledecker plots, are treated in Chapter III-12.
...
%It would help if we knew the sign of the residuals.
Using color, we can add the sign information, for example using blue for
positive, red for negative, and gray for zero residuals.

% Figure~\ref{fig:colorsieve}
...
\index{residual-based shading} residual-based shading
of the tiles to additionally visualize the
...
= 0.05$ and $\alpha = 0.0001$ levels. However, the main purpose of this
shading is not to visualize significance but the \emph{pattern} of
deviation from independence \cite{vcd:Friendly:2000}.
In addition to the shading of the rectangles themselves, the Friendly
shading also
encompasses a choice of line type and line color of the rectangle borders
with similar ideas as described above.

In Figure~\ref{fig:Friendly}, we show again the mosaic for the \data{hospital}
...
\end{center}
\end{figure}
Clearly, the asymmetry for regular and never visited patients,
...
information is already contained in the plot by construction. Nevertheless, using one of the
shadings discussed above will support the analysis process and is therefore recommended.
For example, applying the Friendly shading in Figure~\ref{fig:colorassoc}
...
for the $X^2$ statistic from (\ref{eq:ChiSquare}) was the closed form result
for the distribution. Recently, with the improving perfomance of computers,
conditional inference \index{conditional inference}
(or permutation tests)---carried out either by simulation
...
For testing the independence hypothesis from (\ref{eq:H0}),
using a \index{permutation test} permutation test is particularly intuitive due to the
...
for the size of the residuals), we can add a third piece of information to the plot.
For example, we can visualize the significance of some specified test statistic
(e.g., the $\chi^2$ test statistic)
using less colorful (`uninteresting') colors for non-significant
results. These can again be derived using the same procedure
described in Section~\ref{sec:colors} but using a smaller amount of color, i.e.,
a smaller maximal chroma (e.g., 20 instead of 100).
...
% independence on a significance level of $5\%$ since the $p$ value is $0.1213$.
...
``large''. On the other hand, the test might be significant even
though the residuals are ``small''. In fact, the cut-off points are really
data-dependent. Consider the case of the \data{arthritis} data \index{data!arthritis}
...
gender (see Table~\ref{tab:arthritis} for the female patients):
\begin{table}[h]
\begin{center}
\begin{tabular}{|l|rrr|r|}
\hline
...
of independence is rejected by the $\chi^2$ test even on a $1\%$ level
...
\index{Patefield algorithm}
and computing the maximum statistic for each of these tables.
In Figure~\ref{fig:arthritis2}, we visualize again the \data{arthritis} data, this
time using the maximum test statistic and its 10\% and
1\% critical values as cut-off points.
The shading of the
...
\begin{figure}[p]
\begin{center}
<<arthritis2,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
set.seed(myseed)
...
For the visualization of areas perceptually uniform colors and color palettes
should always be used and HCL color space provides a convenient way to do so.
Shadings can be used to add information to the basic plots and to support the
analysis. Highlighting of tiles can support the analysis of relationships with a
single dependent variable. Residual-based shadings can be used for model diagnostics, e.g.,
by using diverging color palettes to code the sign and size of a residual.
In addition, the non-significance of test statistics can be visualized by using
overall less colorful palettes. Using the maximum instead of the $\chi^2$ (or other) test
statistic(s) allows for data-driven cut-offs in the diverging palettes and
precise diagnostic identification of the cells in conflict with the
null hypothesis of independence.

\section{Selected Methods for Multi-way Tables}
\label{sec:multiway}

In Section~\ref{sec:twoway}, we have presented basic displays
for two-way tables, based on the visualization of information on
...
a way similar to a two-dimensional table.

In the following, we will first treat specialized displays for
exploratory purposes, followed by model-based methods for
conditional independence models.

\subsection{Exploratory Visualization Techniques}
\label{sec:flat}

As an example, consider the well-known UCB admissions data
\index{data!UCB admissions}
\cite{vcd:Bickel+Hammel+O'Connell:1975} on applicants, classified by
admission and gender, to graduate school at the University of California
in Berkeley for the six largest departments in 1973. The `flattened'
contingency table, putting department into the columns and
admission---nested in gender---into the rows, is given by
Table~\ref{tab:ucb}. The table aggregated over all departments gives
wrong evidence of gender bias, i.e., higher admission rates for male
students.
...
\textbf{Gender} & \textbf{Admission} & A & B & C & D & E & F\\\hline
...
A first step in the exploratory analysis of more complex tables is to
get a quick overview on the data. For this,
all basic plots can be combined in pairwise displays,
arranged in a matrix similar to scatterplots in a pairs
plot. \index{pairs plot}
The diagonal cells contain the variable names, optionally with
univariate statistics, whereas the off-diagonal cells feature plots whose
variables are implicitly specified by the cells' positions in the matrix. %'
In Figure~\ref{fig:pairs}, the diagonal cells show bar plots for the
distributions of the variables, and the off-diagonal cells mosaic plots
for the corresponding pairs of variables. The plots suggest that
admission differs between male and female students, and between the departments,
and that the proportion of male and female students varies across the departments.
In particular, departments A and B have a higher proportion of male students
and a lower rejection rate compared to the other departments.
...
pairs(UCBAdmissions)
@
\caption{Pairs plot for the \data{UCB admissions} data.}
\label{fig:pairs}
\end{center}
\end{figure}

The next step is to investigate three-way interactions among the variables.
In this example, we have a binary variable of interest (admission)
that is to be ``explained'' by the others.
A natural way of representing such a three-way table is to use a mosaic
display,
splitting first in the explanatory variables department and gender and highlighting
the resulting mosaic with respect to the dependent variable admissions. Here, we
use the vertical splits for both explanatory variables, resulting in the
\index{doubledecker plot} doubledecker plot
in Figure~\ref{fig:doubleucb}. From the widths of the
tiles it can be seen that students apply to the six departments in
unequal numbers (with a particularly small
number of females in departments~A and B),
and from the highlighting we see again that the admission rates differ
among the departments (roughly speaking, the admission rate is high for A and B,
low for F, and in between for C to E). The rates
are equal for male and female students, except for department A where
\textit{more} female than male students are admitted.

% \setkeys{Gin}{width=0.7\textwidth}
% \begin{figure}
% \begin{center}
% <<ucbhighlight,fig=TRUE,echo=FALSE,height=6,width=9,results=hide>>=
% mosaic(aperm(UCBAdmissions), gp = gpar(fill = grey.colors(2)),
%    spacing = spacing_highlighting, split = TRUE, keep = FALSE, rep = FALSE)
% @
% \caption{Mosaic plot with highlighting for the \data{UCB admissions} data.}
% \label{fig:ucbhighlight}
% \end{center}
% \end{figure}
...
\begin{figure}
\begin{center}
<<ucbhigh,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
doubledecker(aperm(aperm(UCBAdmissions, c(1,3,2))[2:1,,], c(2,3,1)), margins = c(left = 0, right = 5),col = rev(grey.colors(2)))
@
\caption{Doubledecker plot for the \data{UCB admissions} data.}
\label{fig:doubleucb}
\end{center}
\end{figure}

\subsection{Model-based Displays for Conditional Independence Models}
\label{sec:condind}

In addition to the exploratory approaches
to the visualization of multi-way tables outlined in
the previous section, there are specialized displays
designed for the visualization of \index{conditional independence}
conditional independence structures.

One approach is employing pairs plots (such as in Figure~\ref{fig:pairs}) for model search:
we can use the position of the cells in the pairs matrix to select an
independence model and add a corresponding shading to the tiles to
visualize the residuals.
More precisely, each cell $a_{ij}$ in such a matrix defines two
...
% in the lower triangle, and bar charts on the diagonal.
...
with ideas similar to those described in Section~\ref{sec:extensions}.
Thus, mosaic displays can
be extended from purely explorative views to model-based views on the data by
residual-based shadings and association plots can directly visualize deviations
from a given model. Typically, the models considered are log-linear models, however
residual-based shadings are also conceivable for other types of models such as logistic
regression.
In our example, from the exploratory analysis, we already suspect that
admission and gender are independent, given department, except for
department A.
Therefore, a sensible way of representing the table is to
nest admission into department instead of gender to obtain a stratified view.
The corresponding mosaic and association plots are shown
in Figures~\ref{fig:ucbmosaic} and \ref{fig:ucbassoc}. The shading visualizes the
model of admission and gender being conditionally independent, given department.
\setkeys{Gin}{width=\textwidth}
\begin{figure}[t!]
\begin{center}
<<ucbmosaic,fig=TRUE,echo=FALSE,height=6,width=12,results=hide>>=
...
@
\caption{Mosaic plot for the \data{UCB admissions} data.}
\label{fig:ucbmosaic}
\end{center}
\end{figure}
\setkeys{Gin}{width=\textwidth}
\begin{figure}[t!]
\begin{center}
<<ucbassoc,fig=TRUE,echo=FALSE,height=6,width=12,results=hide>>=
assoc(aperm(UCBAdmissions, c(3,2,1)), split = c(TRUE,FALSE,TRUE), shade = TRUE,
residuals_type = "Pearson", residuals = ucb_max$residuals,
gp_args = list(p.value = ucb_max$p.value), rep = c(Admission = FALSE))
@
\caption{Association plot for the \data{UCB admissions} data.}
\label{fig:ucbassoc}
\end{center}
\end{figure}
Clearly, there is no gender bias in the departments except for department A: here,
significantly \textit{more} female students are admitted then expected under independence.
Mosaic displays are already recognized as an excellent
visualization tool for log-linear models
\cite{vcd:Friendly:1999,vcd:Theus+Lauer:1999,vcd:Hofmann:2001}.
Using the flat table representation, the association plot similarly extends from
the simple two-way case to a structured residual plot usable for
the diagnostics of log-linear models. Especially if a
mosaic display contains very small or empty cells, the corresponding association plot
might provide an easier way to detect deviation patterns than the
residual-based shading added to the mosaic plot. Yet another approach
is to plot a mosaic plot of the \emph{expected} frequencies and to highlight
the (relative) residuals in the cells, using different colors for
positive and negative residuals \cite{vcd:Theus+Lauer:1999}.
...
% extramarital sexual experiences, resulting in Table~\ref{tab:presex}.
...
% The mosaic in Fig.~\ref{fig:ucbmosaic} shows the distribution of admission
% and gender for all departments. A larger spacing for
% the department variable has been used to simplify the distinction between the
% groups.
As we have seen, we can use all plots for the analysis of stratified
data by  first using the conditioning variables for splitting.
However, this also displays the marginal distributions of the conditioning variables
which might make it more difficult to interpret the dependent variables of
...
An alternative is to complement the conditioning in the model by
a conditioning in the plot, i.e., to use trellis-like layouts for
\index{trellis layout}
visualizing \emph{partial} tables as defined by the conditioning variables.
All subtables are corrected for the marginals and thus, all
corresponding plots in the panels have the same size.
In addition, trellis layouts help to reduce the complexity of bigger tables.
Figure~\ref{fig:ucbadmissions} visualizes the data by means of a conditional
association plot. Each panel corresponds to a department, and contains
an association plot corresponding to the partial (fourfold) table of admission
...
the situation in department A (more women/less men admitted than
expected under the null hypothesis) causes the rejection of the
hypothesis of conditional independence. In Figure~\ref{fig:ucbadmissions},
data-driven cut-offs (derived from the maximum test for conditional independence)
are used, resulting in a more colorful shading than the fixed cut-offs in
Figure~\ref{fig:ucbmosaic} and \ref{fig:ucbassoc}.

% The overall deviation pattern
...
\begin{figure}
\begin{center}
<<ucbcotab,fig=TRUE,echo=FALSE,height=6,width=10,results=hide>>=
...
@
\caption{Conditional association plot for the \data{UCB admissions}
  data. For each individual association plot, gender is in the rows
  and admission is in the columns.}
\label{fig:ucbadmissions}
\end{center}
\end{figure}

\subsection{A four-way example}

As a four-way example, we use the punishment data \cite{vcd:andersen:1991}
...
punishment of children (see Table~\ref{tab:punish}).
\index{data!Danish corporal punishment}
The data consist of four variables: Attitude is a binary variable
indicating whether a person approves moderate punishment of children
(``moderate''), or refuses any punishment of children (``no'').
Memory indicates whether the person recalls having been
punished as a child. Education indicates the
highest level of education (elementary, secondary, high). Finally,
age indicates the age group in years (15--24, 25--39, $40+$).

\begin{table}[ht]
\begin{center}
\begin{tabular}{|ll|rrrrrrr|}
\hline
 & & \textbf{Age} & \multicolumn{2}{c}{15--24} & \multicolumn{2}{c}{25--39} & \multicolumn{2}{c|}{40+}  \\
\textbf{Education} & \textbf{Attitude} & \textbf{Memory} & Yes & No & Yes & No & Yes & No\\\hline
...
\hline
\end{tabular}
\caption{The \data{punishment} data.}
\label{tab:punish}
\end{center}
\end{table}

In a first step, we create a mosaic plot for an exploratory view of the data:
attitude towards punishment is clearly the response variable
here and should be used as the last split via highlighting.
Furthermore, we expect age and education to have an influence on both memory
and attitude, hence they should be used first for splitting as stratifying
variables. In fact, the question is whether memory has some influence on attitude, given
age and education. We choose to create a three-way mosaic plot of the
hypothesized explanatory variables (first splitting horizontically by age,
then vertically by education, and finally horizontically again by memory),
and to have attitude, the response variable,
highlighted in the tiles (see Figure~\ref{fig:punishhigh}).
We can see that half of the people are more than 40 years of
age, most of whom of only completed elementary school (much more than
in the other age groups). Furthermore, it can be seen that the proportion
of people who recall being punished increases with age, and that
the approval rate decreases with education (this, however, is better shown in
Figure~\ref{fig:punishcotab} described below). As for the question whether
attitude depends on memory, the plot suggests quite clearly (and somewhat
surprisingly) that for those people with elementary school education,
a higher proportion \emph{with} memories tends to
accept moderate punishment of children than those without:
experience of punishment seems to support use of punishment.
For the other education groups, the picture is less clear: some cells
indicate the same association, while others do not.

\setkeys{Gin}{width=0.7\textwidth}
\begin{figure}
\begin{center}
<<punishhigh,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
...
@
\caption{Mosaic plot with highlighting for the \data{punishment} data.}
\label{fig:punishhigh}
\end{center}
\end{figure}

In summary, both age and education clearly matter here.
But is there really any significant influence of memory on attitude towards
punishment apart from that? Or, rephrased as a hypothesis:
Are memory and attitude conditionally independent, given age and education?
To focus on this question, we take another model-based view of the data
which avoids the distracting influence of the marginal variables (age
and education): We employ
partial mosaic plots in a trellis layout (conditioning on age and education) with
residual-based shading and data-driven cut-offs (see Figure~\ref{fig:punishcotab}).
This visualization brings out much more clearly that the increased approval rate
among people with memories of punishment is present mainly in the elementary education
column and, to a lesser degree, in the age 40+ row. Using a
permutation test similar to the one discussed in Section \ref{sec:test},
model statistics reveal that the hypothesis is rejected at a significance level of
1\%, but the shading clearly shows that this association is
only significant in two cells (the two older age groups with elementary education)
where fewer people without memories do not approve punishment than expected under
(conditional) independence. In the group of people with age between
25--39, the lighter shading indicates that here
the association is only significant at the 10\% level.

% ## \setkeys{Gin}{width=0.7\textwidth}
% ## \begin{figure}
% ## \begin{center}
% ## <<punishmosaic,fig=TRUE,echo=FALSE,height=6,width=7,results=hide>>=
% ## punish_test <- coindep_test(punish, margin = c(3, 4))
% ## mosaic(~ age + education + memory + attitude, data = punish, keep = FALSE,
% ##        expected = ~ (memory + attitude) * education * age,
% ##        gp_args = list(interpolate = punish_test$qdist(c(0.9, 0.99))),
% ##        rep = c(attitude = F), set_varnames = c(attitude = "attitude to punishment"),
% ##        set_labels = list(age = c("15-24","25-39","40+")))
% ##
% ## \caption{Mosaic plot for the \data{punishment} data.}
% ## \label{fig:punishmosaic}
% ## \end{center}
% ## \end{figure}

The advantage of the exploratory view
is to visualize the joint distribution of all variables.
On the other hand, the visualization might be
strongly influenced by the marginal distribution of education over age (in particular
the large proportion of 40+ people with elementary education),
which is not of interest in the conditional independence problem. The
partial mosaic plot suppresses this effect by complementing the
conditioning in the model with conditioning in the visualization.

\setkeys{Gin}{width=\textwidth}
\begin{figure}
\begin{center}
<<punishcotab,fig=TRUE,echo=FALSE,height=10,width=10,results=hide>>=
set.seed(123)
cotabplot(punish, panel = cotab_coindep, panel_args = list(varnames = FALSE, margins = c(2, 1, 1, 2)))
@
\caption{Conditional mosaic plot for the \data{punishment} data. For
  each mosaic plot, memory is in the rows (first split) and attitude
  in the columns (second split).}
\label{fig:punishcotab}
\end{center}
\end{figure}
...
Mosaic, association, and sieve plots can be used for visualizing multi-way tables
by applying them to flat representations. Mosaic plots are particularly useful
for exploratory analysis, whereas the other two require specification of a certain
independence model from which deviations should be examined. In addition, specialized
`flavours' of the plots can leverage
exploratory analyses (pairs plots, highlighting, doubledecker plots)
or model-based analyses (residual-based shadings, conditional plots).
...
expected values and the Pearson residuals, respectively. These basic
