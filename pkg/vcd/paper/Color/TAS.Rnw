...
sub-optimal color palettes are used quite frequently for color graphics displayed in journal articles
...
software packages as well as some details on the implementation in \proglang{R}.
...
the \proglang{R} graphics system \citep[see][]{vcd:Murrell:2006},
...
\verb/help("rainbow_hcl", package = "vcd")/. \proglang{R} code for reproducing
